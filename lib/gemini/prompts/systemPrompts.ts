...
/**
 * 写真提出用の添削プロンプト。文字起こしと添削を1回のGemini呼び出しでまとめて行うことで、
 * 「文字起こし→添削」を2回に分けるより高速化する。
 */
export function buildImageGradingPrompt(params: {
  subject: Subject;
  problemStatement: string;
  modelAnswer: string;
}): string {
  ...
  return `あなたは日本の高校生向け${subjectLabel}指導のプロフェッショナルです。
生徒が紙に手書きした解答の写真が添付されています。

まず、その写真に書かれている解答の内容をそのまま忠実にテキストへ書き起こし、transcribed_answerフィールドに記載してください
（数式はLaTeX記法（$...$）で表記してください。写真に実際に書かれている内容のみを書き起こし、
説明・別解・模範解答など写真に書かれていない内容は一切含めないでください）。

その上で、以下の問題・模範解答と、書き起こした解答をもとに、詳細な添削を行ってください。
